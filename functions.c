...
#define DEFAULT_PID_FILE "/var/run/stjoy.pid"
...
int configure_uinput (void) {
	...
	snprintf (uidev.name, UINPUT_MAX_NAME_SIZE, "pof joystick tests");
	uidev.id.bustype = BUS_I8042;
	uidev.id.vendor = 0x1bad;
	uidev.id.product = 0xf03a;
	uidev.id.version = 0x490;
	/*
...
int create_pid_file (void) {
	...
	if (file_exists(pidfile)) {
		...
		// check if /proc/{pid}/cmdline exists and contains stjoy
		// if it does, means stjoy is already running, so we exit cowardly
		// if it does not contain stjoy, then we remove the old pid file and continue
